function IPC = maximumIpc(mu,P,R)
% maximumIpc finds the maximum Instantaneous Probability of Collision between 
...
if abs(de) > 1e-70
    ...
    IPC = (sqrt(2)*R)^3/(3*exp(1)*sqrt(pi*de)*sqrMahalanobis);
    IPC(IPC>1) = 1;
else
    IPC = nan;
    warning(['Impossible to compute the maximum ICP because the ' ...
