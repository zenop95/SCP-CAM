function IPC = projectedIpc(mu,P,R,plane)
% projected computes the projected Instantaneous Collision Probability 
% (IPC) in a plane as the integral of the gaussian probability 
% distribution ellipsoid of the relative position of two bodies described 
% by the mean and covariance matrix in input over the Hard Body Sphere 
% of radius R
...
%        plane = (char) accepted values = ['xy','xz','yz']
...
switch plane
    case 'xy'
        margPdf = @(x,y) integral(@(z) funHndl(x,y,z),-inf,inf, ...
            "ArrayValued",true);
    case 'xz'
        margPdf = @(x,z) integral(@(y) funHndl(x,y,z),-inf,inf, ...
            "ArrayValued",true);
    case 'yz'
        margPdf = @(y,z) integral(@(x) funHndl(x,y,z),-inf,inf, ...
            "ArrayValued",true);
    otherwise 
...
ymin    = @(x)     -sqrt(R^2 - x.^2);
ymax    = @(x)      sqrt(R^2 - x.^2);  
IPC  = integral2(margPdf,xmin,xmax,ymin,ymax,"AbsTol",1e-10,"RelTol",1e-6);
end
