...
int main(void)
{
    ...
    double HBR, order, dt, t_integration, a, e, th0;
   // dummy variables for reading
    ...
    for (int i = 0; i < N+1; i++)
    {
        ...
        DA sqrMaha = dot(xPos,Ppos.inv()*xPos);
        DA probFun = exp(-sqrMaha/2)/sqrt(8*pi*pi*pi*Ppos.det());

        // Perform successive integrations in the last three DA variables
        AlgebraicVector<DA> integMax(9), integMin(9);
        double integExtreme = 0.806*HBR;

        for (int i = 0; i < 8; i++){
            integMax[i] =  DA(i+1);
            integMin[i] =  DA(i+1);}
        integMax[8] = integExtreme;
        integMin[8] = -integExtreme;
        DA IPC = probFun.integ(9);
        IPC = IPC.eval(integMax) - IPC.eval(integMin);

        integMax[7] = integExtreme;  integMax[8] = 0;
        integMin[7] = -integExtreme; integMin[8] = 0;
        IPC = IPC.integ(8);
        IPC = IPC.eval(integMax) - IPC.eval(integMin);

        integMax[6] = integExtreme;  integMax[7] = 0; integMax[8] = 0;
        integMin[6] = -integExtreme; integMin[7] = 0; integMin[8] = 0;
        IPC = IPC.integ(7);
        IPC = IPC.eval(integMax) - IPC.eval(integMin);
